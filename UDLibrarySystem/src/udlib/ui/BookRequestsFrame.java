...
public class BookRequestsFrame extends JFrame {
    ...
    private void approveRequest() {
        ...
        if (row == -1) { JOptionPane.showMessageDialog(this, "Select a request to approve."); return; }
        ...
        try (Connection conn = DBConnection.connect()) {
            ...
            // Update status in book_requests
            ...
            conn.commit();
            JOptionPane.showMessageDialog(this, "✅ Request approved successfully!\nBorrow period until: " + due.format(DateTimeFormatter.ofPattern("MMM dd, yyyy HH:mm")));
            loadRequestsFromDB();
        } catch (Exception e) {
    ...
    private void rejectRequest() {
        int row = table.getSelectedRow();
        if (row == -1) { JOptionPane.showMessageDialog(this, "Select a request to reject."); return; }
