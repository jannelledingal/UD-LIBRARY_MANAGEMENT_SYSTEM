...
public class ManageBooksFrame extends JFrame {
    ...
    private void applyFilters() {
        ...
        // Load all books from DB again
        DefaultTableModel fullModel = new DefaultTableModel(
            new String[]{"Title", "Author", "Category", "Shelf", "ISBN", "Availability", "Cover"}, 0
        );
        ...
        try (Connection conn = DBConnection.connect()) {
            ...
            ModernDialog.showMessage(this, "Error loading data: " + e.getMessage());
            return;
        }

        // Apply LinearSearch (keyword)
        List<Integer> matches = new ArrayList<>();
        if (!searchText.isEmpty()) {
            matches = udlib.algo.AlgorithmUtils.linearSearch(fullModel, searchText);
        } else {
            // if no search keyword, include all
        ...
        // Replace current model with filtered results
        model.setRowCount(0);
        for (Object[] row : filteredRows) model.addRow(row);

        // Sort the final filtered data alphabetically by title
        ...
        if (model.getRowCount() == 0) {
            ModernDialog.showMessage(this, "No matching records found.");
        }
    }
