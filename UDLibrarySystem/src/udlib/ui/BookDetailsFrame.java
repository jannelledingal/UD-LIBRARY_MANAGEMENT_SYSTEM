...
public class BookDetailsFrame extends JFrame {
    ...
    public BookDetailsFrame(String title, String author, String category, String shelf, String isbn, String status) {
        ...
        // Notice label for unavailable books
        JLabel unavailableNotice = new JLabel("⚠ This book is currently not available for borrowing.");
        unavailableNotice.setFont(new Font("Poppins", Font.ITALIC, 14));
        unavailableNotice.setForeground(Color.RED);
        unavailableNotice.setHorizontalAlignment(SwingConstants.CENTER);
        unavailableNotice.setVisible(false);

        // Disable or enable the button based on availability
        if (status.equalsIgnoreCase("Not Available") || status.equalsIgnoreCase("Unavailable")) {
            requestBtn.setEnabled(false);
            requestBtn.setBackground(new Color(230, 230, 230));
            requestBtn.setForeground(Color.GRAY);
            unavailableNotice.setVisible(true);
        } else {
        ...
        buttonRow.setBackground(new Color(227,242,253));
        buttonRow.add(requestBtn);
        buttonRow.add(backBtn);

        buttonPanel.add(buttonRow);
        buttonPanel.add(unavailableNotice);
    ...
    private void loadCoverImage(JLabel label, String isbn) {
        try {
            ...
            if (!coverFile.exists()) coverFile = new File("src/udlib/assets/covers/no_cover1.png");
